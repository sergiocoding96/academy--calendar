...
import type { Whereabouts as WhereaboutsType } from '@/features/player-database/types'

interface PlayerWhereaboutsClientProps {
  playerId: string
  initialWhereabouts: WhereaboutsType[]
}

export function PlayerWhereaboutsClient({
  playerId,
  initialWhereabouts,
}: PlayerWhereaboutsClientProps) {
  const { whereabouts, loading, refetch } = useWhereabouts(playerId, {
    initialData: initialWhereabouts,
  })
